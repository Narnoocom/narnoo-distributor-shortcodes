...
define( 'NARNOO_DISTRIBUTOR_SHORTCODE_PLUGIN_PATH', plugin_dir_path( __FILE__ ) );
...
class Narnoo_Shortcodes {
	...
	**/
	function create_menu(){
		...
		add_menu_page(
			__('Narnoo ShortCode Information', 	NARNOO_DISTRIBUTOR_SHORTCODE_I18N_DOMAIN),
			__('ShortCodes', 			NARNOO_DISTRIBUTOR_SHORTCODE_I18N_DOMAIN),
			'manage_options',
			'narnoo_shortcode_info',
			array( &$this, 'narnoo_shortcode_info' ),
			NARNOO_DISTRIBUTOR_SHORTCODE_URL . 'images/icon-16.png'
			);
	}
	...
	*/
	function narnoo_shortcode_info(){
		ob_start();
		require( NARNOO_DISTRIBUTOR_SHORTCODE_PLUGIN_PATH . 'libs/html/help_info_tpl.php' );
		echo ob_get_clean();
	}
	 ...
	 * Display specified brochure with thumbnail or preview image, link to PDF file and caption.
	 **/
	function narnoo_distributor_slider_shortcode( $atts ) {
		ob_start();
		require( NARNOO_DISTRIBUTOR_SHORTCODE_PLUGIN_PATH . 'libs/narnoo_slider/slider.php' );
		return ob_get_clean();
	}
	...
	function narnoo_operator_slider_shortcode( $atts ) {
		ob_start();
		require( NARNOO_DISTRIBUTOR_SHORTCODE_PLUGIN_PATH . 'libs/narnoo_operator_slider/slider.php' );
		return ob_get_clean();
	}
	/*
	 * Loads Javascript files for tiles gallery shortcode.
	...
	function narnoo_flip_book_shortcode( $atts ) {
		ob_start();
		require( NARNOO_DISTRIBUTOR_SHORTCODE_PLUGIN_PATH . 'libs/narnoo_flip_book/flipbook.php' );
		return ob_get_clean();
	}
	/*
	 * Loads Javascript files for tiles gallery shortcode.
	...
	function narnoo_products_shortcode( $atts ) {
		ob_start();
		require( NARNOO_DISTRIBUTOR_SHORTCODE_PLUGIN_PATH . 'libs/narnoo_products/display.php' );
		return ob_get_clean();
	}
	/*
	 * Loads Javascript files for tiles gallery shortcode.
	 ...
	 * Display specified narnoo imported products.
	 *
	function narnoo_tripadvisor_shortcode( $atts,$content=null ) {
		ob_start();
		require( NARNOO_DISTRIBUTOR_SHORTCODE_PLUGIN_PATH . 'libs/narnoo_tripadvisor/tripadvisor.php' );
		return ob_get_clean();
	}
